...
for indt = 1:ntime
    ...
    % local month
    local_month = ceil(round((mod(time(indt),CONV.sperfrc)/CONV.sperfrc)*MAIN.nforcing));%CONV.spery)/CONV.spery)*12); % and round() added, otherwise +1 at some numbers (eg indt = 29, indt = 56)
  ...
  if MAIN.incl_egg_move
    ...
    flux_in_num_eggs2 = flux_in_num_eggs;
        ...
        elseif strcmp(MAIN.reg_type,'omnT') % regulation
            ...
            elseif length(size(FORC.societenf)) == 3
               %disp("timesoc")
                ...
                societenf = cat(3,squeeze(FORC.societenf(:,:,ceil(indt/MAIN.nforcing))),squeeze(FORC.societenf(:,:,ceil(indt/MAIN.nforcing))),squeeze(FORC.societenf(:,:,ceil(indt/MAIN.nforcing)))); % create identical maps for all three groups%cat(3,squeeze(FORC.societenf(:,:,local_month)),squeeze(FORC.societenf(:,:,local_month)),squeeze(FORC.societenf(:,:,local_month))); % create identical maps for all three groups
                %societenf1 = cat(3,squeeze(FORC.societenf(:,:,indt)),squeeze(FORC.societenf(:,:,indt)),squeeze(FORC.societenf(:,:,indt))); % create identical maps for all three groups
                %disp(ceil(indt/MAIN.nforcing))
            end
            ...
            elseif length(size(FORC.effEtarg)) == 4  % indt >= 12 %
                %effEtarg = squeeze(FORC.effEtarg(:,:,indt,:));  %(:,:,indt/MAIN.nforcing,:));  
            ...
            if strcmp(ECON.reg_timestep,'monthly')
                ...
                effort_change(mask_reg) = ECON.k_e * (revenue(mask_reg) - cost(mask_reg)) ./ (effort(mask_reg) + CONV.epsln) ...
                    .* exp(-societenf(mask_reg)) + ECON.k_s .* (ones(length(mask_reg),1) - exp(-societenf(mask_reg))) .* (effEtarg(mask_reg) * ECON.precaut ...
